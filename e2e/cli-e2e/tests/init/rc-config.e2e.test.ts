...
describe('.rc.json in empty sandbox', () => {
  it.concurrent<CliTest>(
    'should take default params from prompt with flow',
    async ({ root, cli }) => {
      ...
      const { code, stderr } = await cli.waitForClose();
      expect(stderr).toBe('');
      expect(code).toBe(0);

      const rc = JSON.parse(
        readFileSync(join(root, '.user-flowrc.json'), { encoding: 'utf8' }),
      );
      expect(rc).toEqual(DEFAULT_RC);

      expect(
        existsSync(
          join(root, DEFAULT_RC.collect.ufPath, 'basic-navigation.uf.mts'),
        ),
      ).toBeTruthy();
    },
  );
  ...
  it.concurrent<CliTest>(
    'should take params from prompt without flow',
    async ({ root, cli }) => {
      await cli.run('user-flow', ['init', '--interactive'], false);
  ...
  it.concurrent<CliTest>(
    'should take custom params from prompt',
    async ({ root, cli }) => {
      await cli.run('user-flow', ['init', '--interactive'], false);

      await respondToPrompts(cli, [
        [
          'What is the URL to run the user flows for?',
          'https://example.com' + KEYBOARD.ENTER,
        ],
        ['Folder of the user flows?', 'dummy-uf-path' + KEYBOARD.ENTER],
        ['What is the format of user-flow reports?', KEYBOARD.ENTER],
        [
          'What is the directory to store results in?',
          'dummy-out-path' + KEYBOARD.ENTER,
        ],
        ['Setup user flow', KEYBOARD.DECLINE_BOOLEAN],
      ]);
...
describe('.rc.json in initialized sandbox', () => {
  it.concurrent<CliTest>('should validate params from rc', async ({ cli }) => {
    const { code, stderr } = await cli.run('user-flow', [
      'init',
      '--interactive',
      '--format wrong',
    ]);
    ...
    expect(stderr).toContain(
      'Argument: format, Given: "wrong", Choices: "html", "json", "md", "stdout"',
    );
  });

  it.concurrent<CliTest>(
    'should log and ask if specified rc file param -p does not exist',
    async ({ cli }) => {
      await cli.run(
        'user-flow',
        ['init', '--interactive', '--rcPath wrong/path/to/file.json'],
        false,
      );
...
describe('.rc.json in remote sandbox', () => {
  it<CliTest>('should load configuration if specified rc file param -p is given', async ({
    root,
    cli,
    setupFns,
  }) => {
...
async function respondToPrompts(
  cli: CliTest['cli'],
  promptResponses: [prompt: string, respone: string][],
) {
