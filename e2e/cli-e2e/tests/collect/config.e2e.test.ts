...
describe('collect config', () => {
  it<CliTest>('should log LH config from a user flow', async ({
    setupFns,
    cli,
  }) => {
  ...
  it<CliTest>('should throw is invalid path to LH config', async ({
    setupFns,
    cli,
  }) => {
    setupFns.setupRcJson(DEFAULT_RC);
    ...
    expect(stderr).toContain(
      'Error: ./invalid/path/to/config.json does not exist.',
    );
    expect(code).toBe(1);
  });

  it<CliTest>('should load configPath from RC file', async ({
    root,
    setupFns,
    cli,
  }) => {
    ...
    expect(stdout).toContain(
      'LH Configuration ./lh-config.json is used from CLI param or .user-flowrc.json',
    );
    expect(stderr).toBe('');
    expect(code).toBe(0);
  });
