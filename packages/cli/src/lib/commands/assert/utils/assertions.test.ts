...
import { AllPreset as lighthouseAllPreset } from '../presets/all';
import { convertBudgetsToAssertions } from './budgets-converter';
import { getAllAssertionResults } from './assertions';
...
describe('getAllAssertionResults', () => {
  ...
  it('should fail on assertion for audit that did not run', () => {
    const assertions: Assertions = {missing: 'error'};
  ...
  describe('budgets', () => {
    ...
    it('should return assertion results for budgets UI', () => {
      ...
      const lhrs: Result[] = [lhrWithBudget, lhrWithBudget];
      const results = getAllAssertionResults({assertions}, lhrs);
      expect(results).toEqual([
        {
          url: 'http://page-1.com',
    ...
    it('should assert budgets natively', () => {
      ...
      const lhrs: Result[] = [lhrWithResourceSummary, lhrWithResourceSummary];
  ...
  describe('user timings', () => {
    ...
    it('should assert user timing keys', () => {
      ...
      const lhrs: Result[] = [lhrWithUserTimings, lhrWithUserTimings];
      const results = getAllAssertionResults({assertions, includePassedAssertions: true}, lhrs);
      expect(results).toEqual([
        {
          actual: 123,
  ...
  describe('URL-grouping', () => {
    ...
    it('should report for URLs separately', () => {
      const assertions: Assertions = {'first-contentful-paint': ['error', {minScore: 0.9}]};
      const results = getAllAssertionResults({assertions}, lhrs);
      expect(results).toEqual([
        {
          url: 'http://page-1.com',
