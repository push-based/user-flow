...
export function generateMdReport(flowResult: ReducedReport): string {
  ...
  const dateTime = `Date/Time: ${style(
    new Date().toISOString().replace('T', ' ').split('.')[0].slice(0, -3)
  )}  `;
          ...
          if (label === 'Cumulative Layout Shift') {
            return [
              ...
              overBudget === undefined
                ? '-'
                : parseFloat(overBudget).toFixed(2),
            ];
          } else {
            return [
              label + '',
...
function getResourceSizes(
  resourceSizesBudget: Table | undefined
): undefined | string[][] {
  if (resourceSizesBudget === undefined) {
    ...
    return [
      resourceSizesBudget.headings
        .filter((i: any) =>
          ['label', 'transferSize', 'sizeOverBudget'].includes(i.key)
        )
...
function getResourceCounts(
  resourceCountsBudget: Table | undefined
): undefined | string[][] {
  if (resourceCountsBudget === undefined) {
    ...
    return [
      resourceCountsBudget.headings
        .filter((i: any) =>
          ['label', 'requestCount', 'countOverBudget'].includes(i.key)
        )
        .map((h: any) =>
...
export function getStepsTable(
  ...
  const tableStepsArr = formatStepsForMdTable(
    reportCategories,
    reducedReport,
    baselineResults
  );
...
function formatStepsForMdTable(
  reportCategories: string[],
  reducedReport: ReducedReport,
  baselineResults?: any
): string[][] {
  if (baselineResults) {
    const enrichedReducedReport = enrichReducedReportWithBaseline(
      reducedReport,
      baselineResults
    );
    return enrichedReducedReport.steps.map((step) => {
      const results = reportCategories.map((category) =>
        extractEnrichedResults(step, category)
      );
  ...
  return reducedReport.steps.map((step) => {
    const results = reportCategories.map((category) =>
      extractResultsValue(step.results[category])
    );
...
function extractTableHead(reportCategories: string[]): string[] {
  return ['Step Name', 'Gather Mode'].concat(
    reportCategories.map((c) =>
      c
        .split('-')
        .map((cN) => cN[0].toUpperCase() + cN.slice(1))
        .join(' ')
    )
  );
}
...
function extractEnrichedResults(
  step: ReducedFlowStep,
  category: string
): string {
...
function resultWithBaselineComparison(
  result: string,
  baseline: string
): string {
  if (result === baseline) {
