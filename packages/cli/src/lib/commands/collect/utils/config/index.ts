...
export function getLhConfigFromArgv(
  rc: Partial<
    Pick<CollectCommandArgv, 'configPath' | 'config' | 'budgets' | 'budgetPath'>
  >
): LhConfigJson {
  ...
  if (rc?.configPath) {
    ...
    logVerbose(
      `LH Configuration ${rc.configPath} is used from CLI param or .user-flowrc.json`
    );
  } else if (rc?.config) {
    cfg = rc.config;
    logVerbose(
  ...
  if (rc?.budgetPath) {
    ...
    logVerbose(
      `LH Performance Budget ${rc.budgetPath} is used from CLI param or .user-flowrc.json`
    );
  } else if (rc?.budgets) {
