...
export const PROMPT_PERSIST_FORMAT =
  'What is the format of user-flow reports? (use ⬇/⬆ to navigate, and SPACE key to select)';
export const ERROR_PERSIST_FORMAT_REQUIRED =
  'format is required. Either through the console as `--format` or in the `.user-flow.json`';
export const ERROR_PERSIST_FORMAT_WRONG = (wrongFormat: string) => {
  return `Argument: format, Given: "${wrongFormat}", Choices: ${REPORT_FORMAT_VALUES.map(
    (v) => '"' + v + '"'
  ).join(', ')}`;
