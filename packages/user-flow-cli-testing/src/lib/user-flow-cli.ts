...
export class UserFlowCliProject extends CliProject<RcJson> {
  ...
  override async _setup(cfg: UserFlowProjectConfig): Promise<void> {
    ...
    cfg.rcFile = cfg.rcFile || {
      [this.envPreset?.rcPath]: SANDBOX_BASE_RC_JSON,
    };
    ...
    if (
      typeof cfg.rcFile === 'object' &&
      Object.entries(cfg.rcFile).length > 0
    ) {
  ...
  $init(
    processParams?: Partial<InitCommandArgv & GlobalOptionsArgv>,
    userInput?: string[]
  ): Promise<TestResult> {
    const prcParams: ProcessParams = {
      _: 'init',
      ...processParams,
  ...
  $collect(
    processParams?: Partial<CollectCommandArgv & GlobalOptionsArgv>,
    userInput?: string[]
  ): Promise<TestResult> {
    const prcParams: ProcessParams = {
      _: 'collect',
  ...
  readBudget(
    budgetName: string = LH_NAVIGATION_BUDGETS_NAME_DEFAULT
  ): Budget[] {
  ...
  readOutput(
    userFlowName: string,
    format: ReportFormat | undefined = undefined,
    rcFileName: string = DEFAULT_RC_NAME
  ): FileResult[] {
    const outputFiles = readdirSync(this.outputPath());
    const reportPaths =
      outputFiles.filter((name) => {
        if (format) {
  ...
  outputPath(
    ...
    if (!this.rcFile[rcFileName]) {
      throw new Error(
        `Rc file "${rcFileName}" does not exist in: ${Object.keys(
          this.rcFile
        ).join(', ')}`
      );
    }
  ...
  readUserFlow(
    userFlowName: string = DEFAULT_PERSIST_OUT_PATH,
    rcFileName: string = DEFAULT_RC_NAME
  ): string[][] {
  ...
  userFlowPath(
    ...
    return join(
      this.root,
      this.rcFile[rcFileName].collect.ufPath,
      userFlowName
    );
  }
}
...
export function withUserFlowProject<T extends {}>(
  ...
  return withProject<T>(
    cfg,
    fn as any,
    UserFlowCliProjectFactory.create as any
  );
}
