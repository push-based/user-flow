...
describe('target generator', () => {
  ...
  beforeEach(() => {
    ...
    addProjectConfiguration(appTree, PROJECT_NAME, {
      root: projectRoot,
      projectType: 'library',
  ...
  it('should run successfully', async () => {
    ...
    normalizedOptions = normalizeOptions(appTree, baseOptions);
    let projectJson = readJson(
      appTree,
      joinPathFragments(normalizedOptions.projectRoot, 'project.json'),
    );
    expect(projectJson?.targets['user-flow']).toBeUndefined();
    await generator(appTree, baseOptions);
    projectJson = readJson(
      appTree,
      joinPathFragments(normalizedOptions.projectRoot, 'project.json'),
    );
  ...
  it('should add user-flow target to project.json', async () => {
    ...
    expect(packageJson.targets[options.targetName].executor).toBe(
      '@push-based/user-flow-nx-plugin:user-flow',
    );
  });
  ...
  it('should throw if user-flow target already exists in project.json', async () => {
    ...
    updateJson(
      appTree,
      joinPathFragments(opt.projectRoot, 'project.json'),
      (json) => {
        json.targets[opt.targetName] =
          '@push-based/user-flow-nx-plugin:user-flow';
        return json;
      },
    );
    await expect(() => generator(appTree, options)).rejects.toThrow(
      `Target ${opt.targetName} already exists`,
    );
  });
  ...
  it('should throw if user-flowrc.json already exists in project root', async () => {
    ...
    writeJson(
      appTree,
      joinPathFragments(opt.projectRoot, '.user-flowrc.json'),
      {},
    );
    await expect(() => generator(appTree, options)).rejects.toThrow(
      `.user-flowrc.json already exists in ${opt.projectRoot}`,
    );
  });
});
